...
@Import({
  ...
  FeatureFlagWebMvcConfig.class,
  FeatureFlagInterceptor.class,
  FeatureFlagRequestOverrideInterceptor.class
})
@TestPropertySource(
    properties = {
...
class FeatureFlagExampleControllerTest {
  ...
  void shouldReturnRequestOverrideState() throws Exception {
    mockMvc
        .perform(get("/feature-flags/example-feature").param("featureFlag", "FEATURE:true"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.featureFlag").value("FEATURE"))
  ...
  void shouldExposeGatedEndpointForEnabledRequest() throws Exception {
    mockMvc
        .perform(get("/feature-flags/example-feature/gated").param("featureFlag", "FEATURE:true"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.featureFlag").value("FEATURE"))
        .andExpect(jsonPath("$.enabled").value(true));
  }
