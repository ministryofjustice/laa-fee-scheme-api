...
class FeeCodesControllerTest {
  ...
  @MockitoBean
  private FeeCodesService feeCodesService;
  ...
  void getFeeCodesByAreaOfLaw() throws Exception {
    ...
    FeeCodesResponseV1 response =
            FeeCodesResponseV1.builder()
                    .feeCodes(List.of(feeCodeDetails))
                    .build();

    when(feeCodesService.getFeeCodesV1("LEGAL_HELP"))
            .thenReturn(response);

    mockMvc.perform(get("/api/v1/fee-codes/LEGAL_HELP")
  ...
  void getFeeCodesByAreaOfLawMultipleCodes() throws Exception {
    ...
    FeeCodesResponseV1 response =
            FeeCodesResponseV1.builder()
                    .feeCodes(List.of(feeCodeDetails, feeCodeDetails2))
                    .build();
  ...
  void getFeeCodesV1ByAreaOfLawThrowsExceptionWhenAreaOfLawNotFound() throws Exception {
    when(feeCodesService.getFeeCodesV1("FEE123")).thenThrow(new AreaOfLawNotFoundException("FEE123"));

    mockMvc.perform(get("/api/v1/fee-codes/FEE123")
                    .contentType(MediaType.APPLICATION_JSON))
