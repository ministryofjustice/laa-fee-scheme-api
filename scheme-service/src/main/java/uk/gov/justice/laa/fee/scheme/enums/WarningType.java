...
public enum WarningType {
  ...
  WARN_CLAIM_AGNST_PUB_AUTH_ESCAPE_THRESHOLD(
      "WAROTH2", getEscapeMessage(), CategoryType.CLAIMS_PUBLIC_AUTHORITIES),

  WARN_CLINICAL_NEGLIGENCE_ESCAPE_THRESHOLD(
      "WAROTH3", getEscapeMessage(), CategoryType.CLINICAL_NEGLIGENCE),
  ...
  WARN_PRISON_MAY_HAVE_ESCAPED(
      "WARCRM5",
      "Profit Costs and Waiting combined exceed the Lower Standard Fee Limit. A higher standard fee may be claimable instead. "
  ...
  public static List<WarningType> getByCategory(CategoryType categoryType) {
    return Arrays.stream(values()).filter(e -> e.getCategoryType() == categoryType).toList();
  }
}
